...
class ApiNHN:
    ...
    def __init__(self):
        ...
        result = requests.get(config.NHN_ZONE.format(config.NHN_APIKEY), headers=config.HEADER)
        if result.status_code == 200:
            ...
            raise Exception('NHN api를 가져올 수 없습니다.')
    ...
    def recordlist(self, zone):
        ...
        while True:
            query['page'] = cnt
            zone_row_result = requests.get(config.NHN_RECORD.format(config.NHN_APIKEY, self.zoneInfo[zone]),headers=config.HEADER,params=query)
